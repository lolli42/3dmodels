...
module feederMicroSwitchHolder() {
    union() {
        ...
        // A latch as filament guide towards switch for easier loading
        translate([
            0,
            -2*(2*switchHolderBaseMaterialZ + switchX - switchHolderScrewLatchY/2),
            filamentFlowSpaceZ/2 + switchHolderBaseMaterialZ + switchHolderInsetZ
        ]) rotate([
            0,
            90,
            0
...
translate([1, 0, 0]) feederMicroSwitchHolder();
translate([-1, 0, 0]) mirror([1, 0, 0 ]) feederMicroSwitchHolder();
